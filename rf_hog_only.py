...
class FeatureExtractor:
    """Extract HOG (Shape) features only"""

    def __init__(self, img_size=(224, 224), hog_config=None):
...
class RFSportsClassifier:
    """Random Forest classifier for sports image classification (HOG only)"""
    ...
    def load_dataset(self, data_dir, max_samples_per_class=None):
        ...
        print(f"Extracting HOG features from {data_dir}...")
    ...
    def train(self, train_dir, n_estimators=200, max_depth=None,
        ...
        print("\n" + "=" * 80)
        print("TRAINING RANDOM FOREST CLASSIFIER (HOG ONLY)")
        print("=" * 80)
        ...
        # Apply PCA (mandatory for high-dim HOG)
        if self.use_pca:
            print(f"\nApplying PCA...")
    ...
    def predict(self, test_dir, max_samples_per_class=None):
        print("\n" + "=" * 80)
        print("TESTING RANDOM FOREST CLASSIFIER (HOG ONLY)")
        print("=" * 80)
    ...
    def evaluate(self, y_true, y_pred, save_dir='rf_results_hog'):
        print("\n" + "=" * 80)
        print("EVALUATION RESULTS (HOG ONLY - RANDOM FOREST)")
        print("=" * 80)
    ...
    def _plot_confusion_matrix(self, cm, save_dir):
        ...
        plt.title('Confusion Matrix - HOG Only (Random Forest, Normalized)', fontsize=20)
    ...
    def save_model(self, save_path='rf_model_hog.pkl'):
        model_data = {
...
def rf_hyperparameter_search(train_dir, val_dir, save_dir='rf_search_hog'):
    print("\n" + "=" * 80)
    print("FAST RANDOM FOREST HYPERPARAMETER SEARCH - HOG FEATURES")
    print("=" * 80)
    ...
    print(f"Searching n_estimators={n_estimators_list}, max_depth={max_depth_list}, max_features={max_features_list}")

    for n_estimators in n_estimators_list:
        for max_depth in max_depth_list:
            for max_features in max_features_list:

                print(f"\nTesting n_estimators={n_estimators}, max_depth={max_depth}, max_features={max_features}...")
                try:
                    ...
                    print(f"Error in config (n_estimators={n_estimators}, "
                          f"max_depth={max_depth}, max_features={max_features}): {e}")

    if not results:
        raise RuntimeError("No successful Random Forest runs in the HOG hyperparameter search.")

    results_df = pd.DataFrame(results)
    results_path = os.path.join(save_dir, 'rf_search_results_hog.csv')
    ...
    with open(os.path.join(save_dir, 'best_rf_config_hog.json'), 'w') as f:
...
def main():
    ...
    try:
        import skimage  # just to ensure dependency
    except ImportError:
    ...
    if run_search:
        ...
        print("\n" + "=" * 80)
        print("TRAINING FINAL RANDOM FOREST MODEL WITH BEST CONFIG (HOG ONLY)")
        print("=" * 80)
    ...
    classifier.save_model('rf_sports_classifier_hog.pkl')
    y_test_true, y_test_pred = classifier.predict(test_dir, max_samples_per_class=max_samples)
    classifier.evaluate(y_test_true, y_test_pred, save_dir='rf_results_hog')
