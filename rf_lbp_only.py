...
class RFSportsClassifier:
    """Random Forest classifier for sports image classification (LBP only)"""
    ...
    def train(self, train_dir, n_estimators=200, max_depth=None,
        ...
        print("\n" + "=" * 80)
        print("TRAINING RANDOM FOREST CLASSIFIER (LBP ONLY)")
        print("=" * 80)
        ...
        # Apply PCA (optional but helps reduce noise)
        if self.use_pca:
            print(f"\nApplying PCA...")
    ...
    def predict(self, test_dir, max_samples_per_class=None):
        print("\n" + "=" * 80)
        print("TESTING RANDOM FOREST CLASSIFIER (LBP ONLY)")
        print("=" * 80)
    ...
    def evaluate(self, y_true, y_pred, save_dir='rf_results_lbp'):
        print("\n" + "=" * 80)
        print("EVALUATION RESULTS (LBP ONLY - RANDOM FOREST)")
        print("=" * 80)
    ...
    def _plot_confusion_matrix(self, cm, save_dir):
        ...
        plt.title('Confusion Matrix - LBP Only (Random Forest, Normalized)', fontsize=20)
    ...
    def save_model(self, save_path='rf_model_lbp.pkl'):
        model_data = {
...
def rf_hyperparameter_search(train_dir, val_dir, save_dir='rf_search_lbp'):
    print("\n" + "=" * 80)
    print("RANDOM FOREST HYPERPARAMETER SEARCH - LBP FEATURES")
    print("=" * 80)

    os.makedirs(save_dir, exist_ok=True)

    # Recommended grid with depth control for LBP
    ...
    results_path = os.path.join(save_dir, 'rf_search_results_lbp.csv')
    ...
    with open(os.path.join(save_dir, 'best_rf_config_lbp.json'), 'w') as f:
...
def main():
    ...
    if run_search:
        ...
        print("\n" + "=" * 80)
        print("TRAINING FINAL RANDOM FOREST MODEL WITH BEST CONFIG (LBP ONLY)")
        print("=" * 80)
    ...
    classifier.save_model('rf_sports_classifier_lbp.pkl')
    y_test_true, y_test_pred = classifier.predict(test_dir, max_samples_per_class=max_samples)
    classifier.evaluate(y_test_true, y_test_pred, save_dir='rf_results_lbp')
