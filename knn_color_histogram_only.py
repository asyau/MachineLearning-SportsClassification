...
class KNNSportsClassifier:
    ...
    def train(self, train_dir, n_neighbors=5, metric='euclidean', weights='uniform',
        ...
        print("\n" + "="*80)
        print("TRAINING K-NN CLASSIFIER (COLOR HISTOGRAM ONLY)")
        print("="*80)
    ...
    def predict(self, test_dir, max_samples_per_class=None):
        print("\n" + "="*80)
        print("TESTING K-NN CLASSIFIER (COLOR HISTOGRAM ONLY)")
        print("="*80)
    ...
    def evaluate(self, y_true, y_pred, save_dir='knn_results_color'):
        print("\n" + "="*80)
        print("EVALUATION RESULTS (COLOR HISTOGRAM ONLY)")
        print("="*80)
    ...
    def _plot_confusion_matrix(self, cm, save_dir):
        ...
        plt.title('Confusion Matrix - Color Histogram Only (Normalized)', fontsize=20)
    ...
    def save_model(self, save_path='knn_model_color.pkl'):
        model_data = {
...
def k_only_hyperparameter_search(train_dir, val_dir, save_dir='knn_k_search_color'):
    print("\n" + "="*80)
    print("K-NN HYPERPARAMETER SEARCH (K ONLY) - COLOR HISTOGRAM FEATURES")
    print("="*80)
...
def main():
    ...
    if run_k_search:
        ...
        print("\n" + "="*80)
        print("TRAINING FINAL MODEL WITH BEST CONFIG (COLOR HISTOGRAM ONLY)")
        print("="*80)
    ...
    classifier.save_model('knn_sports_classifier_color.pkl')
    y_test_true, y_test_pred = classifier.predict(test_dir, max_samples_per_class=max_samples)
    classifier.evaluate(y_test_true, y_test_pred, save_dir='knn_results_color')
